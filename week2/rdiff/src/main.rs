...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..m {
        for j in 0..n {
            ...
            let str2 = &seq2[j];
            if str1.eq(str2) {
                let tmp = c.get(i, j).unwrap();
                c.set(i+1, j+1, tmp+1).unwrap();
            } else {
                let tmp = cmp::max(c.get(i+1, j).unwrap(), c.get(i, j+1).unwrap());
                c.set(i+1, j+1, tmp).unwrap();
            }
        }
    }
    c
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("Failed to open file");
    let lines2 = read_file_lines(filename2).expect("Failed to open file");
